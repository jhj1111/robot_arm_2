...
def main(args=None):
    ...
    def pick_obj(x, y):
        ...
        print(f'sol = {sol}')
        movejx(posx(x, y, H_top_, rx, ry, rz), sol=sol) #컵 12개 전부 쌓여있을 때의 높이로 이동
        joint_close()
        ...
        if i_num == len(goal_points):   #마지막 컵 돌리기
            ...
            movel(posx(get_current_posx()[0][:3] + [90, -90, 90]), vel=10, acc=10)  # 툴 90도 회전
            _, new_sol = get_current_posx()
            #print(f'new_sol = {new_sol}')
            #sol = new_sol

            movejx(posx([0, 0, -13, 0, 0, 0]), mod=DR_MV_MOD_REL, sol=sol)  # 물체를 집기 위한 높이까지 이동
    ...
    def put_obj(x, y, z):
        ...
        movejx(posx(x, y, 88.45 + z + offset, rx, ry, rz), sol=sol)
    ...
    while rclpy.ok() and i_num <= len(goal_points):
        ...
        setvel(20, 20)
        setvelj(20, 20)
        movej([0, 0, 90, 0, 90, 0], r=200)  # 초기 위치

        pick_obj(*start)
        if i_num == len(goal_points) : #컵 뒤집기
            list(goal_points[i_num-1])[2] += 13 #마지막 포인트 - 마지막 컵 위치에서 z +13 상단
